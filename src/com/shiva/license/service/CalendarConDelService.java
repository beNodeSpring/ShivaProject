...
public class CalendarConDelService implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			//dao에서 인서트 처리
			result=boarddao.WorkdayDelete(boardname,itc_name);
			
			//실패시 펄스
			if(result==false) {
				System.out.println("일정 삭제실패");
				forward.setRedirect(false);
				//이동
				forward.setPath("AdminPage.lo?action=delfail&year="+year+"&month="+month);
				return forward;
			}
			System.out.println("일정 삭제성공");
