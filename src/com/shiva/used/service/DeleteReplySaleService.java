...
public class DeleteReplySaleService implements Service {
	...
	public ServiceForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		boolean usercheck = useddao.isBoardReplyWriter(num, ID);
		if(!usercheck) {
			response.setContentType("text/html;charset=utf-8");
			PrintWriter out = response.getWriter();
			out.println("<script>");
			out.println("alert('수정할 권한이 없습니다.');");
			out.println("location.href = './DetailSaleService.uo?num="+num+"'");
			out.println("</script>");
			out.close();
			return null;
		}else{
			boolean boardDelete = useddao.boardReplyDelete(num,content);
			if(boardDelete) {
				response.setContentType("text/html;charset=utf-8");
				PrintWriter out = response.getWriter();
				out.println("<script>");
